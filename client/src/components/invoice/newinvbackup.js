...
class NewInvoice extends Component {
  ...
  componentDidMount () {
    ...
    this.getInvoice(id)
      .then(invoice => {
        const { data } = invoice
        const { entries, headers, type = InvoiceTypes[0].value, description } = data
  ...
  render () {
    ...
    return (
      <Segment basic>
        ...
        <Header as='h4'>
          <Dropdown onChange={this.handleTypeChange.bind(this)} selection value={type} options={InvoiceTypes} />
        </Header>
